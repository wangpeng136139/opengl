...
RTTR_REGISTRATION
{
registration::class_<TestMeshRenderer>("TestMeshRenderer")
.constructor<>()(rttr::policy::ctor::as_raw_ptr);
}


TestMeshRenderer::TestMeshRenderer() {
...
void TestMeshRenderer::SetMVP(glm::mat4 mvp) {
    mvp_ = mvp;
}

void TestMeshRenderer::SetMaterial(Material* material) {
    material_ = material;
}

void TestMeshRenderer::SetMeshFilter(MeshFilter* mesh_filter) {
    mesh_filter_ = mesh_filter;
}


void TestMeshRenderer::Render() {
...
void TestMeshRenderer::TestRender() {
    //主动获取 Transform 组件，计算mvp。
    auto component_transform = game_object()->GetComponent("Transform");
    auto transform = dynamic_cast<Transform*>(component_transform);
    if (!transform) {
    ...
    glUseProgram(gl_program_id);
    {
        ...
        glBindVertexArray(vertex_array_object_);
        {
            glDrawElements(GL_TRIANGLES, mesh_filter_->GetMesh()->vertex_index_num_, GL_UNSIGNED_SHORT, 0);//使用顶点索引进行绘制，最后的0表示数据偏移量。
        }
        glBindVertexArray(0);
    }
}

void TestMeshRenderer::MVPRender() {
    //获取`Shader`的`gl_program_id`，指定为目标Shader程序。
    GLuint gl_program_id = material_->shader()->GetProgramId();
    if (vertex_array_object_ == 0) {
        GLint vpos_location = glGetAttribLocation(gl_program_id, "a_pos");
